...
npx wrangler d1 execute ielts_vocab --remote --file=migrations/002-review.sql   # 复习功能字段（增量）
npx wrangler d1 execute ielts_vocab --remote --file=migrations/003-srs.sql      # SRS 调度 + 复习日志（增量）
```
...
├── migrations/             # 已部署旧库的增量迁移（001 多用户 / 002 复习字段 / 003 SRS 调度+复习日志）
├── scripts/
